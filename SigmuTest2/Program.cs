...
namespace SigmuTest2
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var transTime = db.F_SP_PROCESS.Where(x => x.SP_NAME == "F_CUST_ACCOUNT_CRM");
            var Url = ConfigurationSettings.AppSettings[@"ConnectAPI"];
            foreach (var t in transTime)
            {
                var lastTransTime = Convert.ToDateTime(t.LAST_TRANS_VALUE);
                // 比較時間
